...
import { Skill, Variable, runSkill, fetchSkills } from '../api';
...
async function fillVariable(
  ...
  const value = await vscode.window.showInputBox({
    ...
    ignoreFocusOut: true,
    validateInput: val => {
      if (v.required && !val?.trim()) return `${label} is required`;
      return null;
    },
  });
...
async function deliverPrompt(prompt: string, skillTitle: string): Promise<void> {
  const mode = vscode.workspace.getConfiguration('cpg').get<string>('insertMode') || 'clipboard';

  if (mode === 'clipboard') {
    await vscode.env.clipboard.writeText(prompt);
    vscode.window.showInformationMessage(
      `CPG: "${skillTitle}" copied to clipboard ✓`,
      'Open Panel'
    ).then(action => {
      if (action === 'Open Panel') showPromptPanel(prompt, skillTitle);
    });
...
function showPromptPanel(prompt: string, title: string): void {
...
export async function runSkillCommand(
  ...
  if (config.get<boolean>('autoDetectLanguage') && language) {
  ...
  for (const v of vars) {
    ...
    // User pressed Escape
    if (value === undefined && v.required) return;
  ...
  await vscode.window.withProgress({
    ...
    const result = await runSkill(apiKey, slug, variables);

    if (!result.ready && result.unfilled_variables?.length) {
      vscode.window.showWarningMessage(
        `CPG: Missing variables: ${result.unfilled_variables.join(', ')}`
      );
      return;
    }
